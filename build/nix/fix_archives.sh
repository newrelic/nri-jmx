...
find dist -regex ".*_dirty\.tar.gz" | while read tarball_dirty; do
  ...
  mv ${TARBALL_CONTENT_PATH}/nri-${INTEGRATION} "${TARBALL_CONTENT_PATH}/opt/newrelic-infra/newrelic-integrations/bin/"
  ...
  mv ${TARBALL_CONTENT_PATH}/jvm-metrics.yml.sample ${TARBALL_CONTENT_PATH}/etc/newrelic-infra/integrations.d/
